  ...
  return exports.connectDB().then(() => {
    ...
    return new Promise((resolve, reject) => {
      db.run("INSERT INTO Category ( CategoryID, CategoryName, Reusable ) " +
        "VALUES ( ?, ? , ? );", [categoryid, categoryname, reusable], err => {
          if (err) reject(err);
          else {
  ...
  return exports.connectDB().then(() => {
    ...
    return new Promise((resolve, reject) => {
      db.run("UPDATE Category " +
        ...
        "WHERE CategoryID = ?", [categoryname, reusable, categoryid], err => {
          if (err) reject(err);
          else {
...
exports.read = function(categoryid) {
  return exports.connectDB().then(() => {
    return new Promise((resolve, reject) => {
      db.get("SELECT * FROM Category WHERE CategoryID = ?", [categoryid], (err, row) => {
...
exports.delete = function(categoryid) {
  return exports.connectDB().then(() => {
    return new Promise((resolve, reject) => {
      db.run("DELETE FROM Category WHERE CategoryID = ?;", [categoryid], err => {
        if (err) reject(err);
        else {
